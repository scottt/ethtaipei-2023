...
import { WebSocketTransport, IWebSocketCtor } from './websocket-transport';
...
export class LightWalletProvider {
  ...
  constructor (fetch: any, abortControllerCtor: any, webSocketCtor: IWebSocketCtor,
    web3AccountsInstance: Accounts, options: any, privateKeys: string[], rpcUrl: string, addressIndex: number, numAddresses: number) {
    ...
    if (rpcUrl.startsWith('http')) {
      ...
      this.transport = new WebSocketTransport(webSocketCtor, rpcUrl, options);
    }
